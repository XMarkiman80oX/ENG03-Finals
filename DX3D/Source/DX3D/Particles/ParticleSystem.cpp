...
void ParticleSystem::createRenderingResources(GraphicsEngine& graphicsEngine)
{
    ...
    // Note: We're using the color field to store UV coordinates (first 2 components)
    ...
    auto device = renderSystem.getDeviceContext().getDeviceContext()->GetDevice();
    ...
    // Store the raw pointers for rendering (we'll manage them separately)
    ...
    m_instanceBufferCapacity = 10000; // Start with capacity for 10k particles
